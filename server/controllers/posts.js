module.exports = {
    ...
    createPost: async (req, res) => {
        ...
        // TODO: didn't work; id associated with req.session.user was undefined. This way at least allows a post :(
        const date = new Date()
        const db = await req.app.get('db')
        if (id) {
            const userPost = await db.post.create_post([
                id,
                title,
                img,
                content,
            ...
            res.status(403).send('It is forbidden.')
        }
    },
    readPost: (req, res) => {
        req.app
